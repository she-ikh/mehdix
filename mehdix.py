...
"""%s
 ...
 - author      : Mehdi Hasan
 - facebook    : facebook.com/mehdihasan20
 - whatsap     : +8801932347250
 - github      : github.com/she-ikh
 - script name : she-ikh
 - version     : 1.9
...
class ngentod:
    ...
    def langsung(self):
        ...
        if suuu == '':
            ...
            print '\n %s[%s∆%s] mehdi %sOK%s tersimpan di >%s mehdi/OK-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.2)
            print '%s [%s∆%s] mehdi %sCP %stersimpan di > %s mehdi/CP-%s-%s-%s.txt'%(P,K,P,K,P,K,ha, op, ta);jeda(0.2)
            print '\n    %s>>>>>>>>>>>> %sSemoga Hari Mu suram %s<<<<<<<<<<<<\n'%(P,K,P);jeda(0.2)
            with ThreadPoolExecutor(max_workers=30) as log:
            	for akun in self.id: 
                    try:
...
def menu():
    ...
    kotak()
#    banner()
    print (' %s[∆] Auhtor    : Mehdi Hasan'%(P))
    print (' [∆] gh auhtor : https://github.com/she-ikh')
    print (' [∆] gh auhtor : https://github.com/mehdi)
    print (jembut)
